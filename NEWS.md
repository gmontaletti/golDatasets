...
* `build_inapp_focus_long()`: orchestratore che invoca i mapper su tutti
  i CSV in `INAPP GOL/focus_gol_all/<report_id>/`.
* `.map_tab_1_5()`: tabella 1.5 "patto di servizio attivo per target ×
  ...
  `Totale, SFL_domanda_accolta, ADI_attivabili, NASpI_domanda_presentata,
  Altri_disoccupati`.
* `.map_tab_2_3()`: tabella "corsi di formazione competenze digitali"
  per dimensione (area geografica, percorso, eta', genere, ...) -
  **88 righe** estratte (solo report 17/2025 ha la struttura canonica).
...
* **Tab 1.3 / 1.4** caratteristiche socio-anagrafiche per regione: 22+
  righe × 12 report disponibili, ma duplicano informazione già nel
  tema B del decoder storico. Va decisa la strategia di deduplicazione.
...
* Tema B (caption 3, 1.3, 1.4): mapping deterministico delle 12
  colonne anagrafiche (`Maschi/Femmine/Totale`, `15-29/30-54/55+/Totale`,
...
Vectorizzazione di `classify_unita()` per consentire l'uso in
data.table con vettori di righe.

## Risultati

* `storico_decoder`: 228 righe; distribuzione `low` ridotta da
  **53 a 20** (-62%). Tema B: 0 low residui. Tema F: 0 low residui.
  Tema H: 17 low (-37%).
* `gol_storia_caratteristiche`: 4.262 -> **6.996 righe** (+64%),
  grazie ai 27 mapping anagrafici di tema B ora classificati `high`.
* `gol_storia_esiti`: 13.561 -> **15.427 righe** (+14%), grazie
  alle nuove variabili F e H decodificate.
...
* Tab 1.3, 1.4, 1.5, 2.3: estrazione `pdfplumber` produce 1-5 righe
  per report (header multi-riga non parsato), inutilizzabile senza un
  parser PDF specializzato.
* Tab 1.6, 1.7: estraggono 20-36 righe ma con strutture incompatibili
  con le ipotesi del piano (1.6 non e' per regione ma trasversale
  per target × caratteristica; 1.7 ha 38 colonne irregolari).
...
* `gol_decode_storico()`: applica il decoder a
  `gol_storico_regionale`, aggiungendo `era` e le colonne
  semantiche. Supporta filtro `min_confidenza` (high / medium / low).
...
Tre funzioni di estrazione compatibili con `plot_timeline()`:
...
* Vignette `merge-gol-cob` estesa con sezione "Storia lunga 2022-2025"
  che mostra 3 grafici (presi in carico totale, composizione di
  genere, LEP-E) con cadenze miste (episodica 2022 → mensile 2025) e
  rotture annotate.

## Future work (v0.6.0)

* Mapper per le 6 nuove tabelle INAPP estratte in `focus_gol_all/`
  (1.3, 1.4, 1.5, 1.6, 1.7, 2.3) — patto di servizio, vulnerabilita',
  competenze digitali.
* Refinement manuale del CSV `storico_decoder.csv` per ridurre la
  quota di righe `confidenza = low` (53/228).
